...
<template>
  <view class="h-screen flex flex-col items-center pt-32 px-6 bg-white">
    ...
    <view v-if="!userInfo" class="w-full flex flex-col items-center">
      ...
      <text class="mt-4 text-xs text-gray-400 text-center"> 登录即代表同意用户协议与隐私政策 </text>
    </view>
    ...
    <view v-else class="flex flex-col items-center gap-6 animate-fade-in mt-10 w-full px-10">
      <!-- 头像修改 -->
      <view class="relative">
        <button class="p-0 border-0 bg-transparent flex flex-col items-center overflow-visible after:border-0" open-type="chooseAvatar" @chooseavatar="onChooseAvatar">
          <image :src="userInfo.avatarUrl" class="w-24 h-24 rounded-full border-4 border-gray-100 shadow-md" mode="aspectFill" />
      ...
      <view class="w-full bg-gray-50 rounded-lg px-4 py-3 flex items-center">
        ...
        <input type="nickname" class="flex-1 text-gray-900 font-medium" :value="userName" placeholder="请输入昵称" @blur="onNicknameBlur" @confirm="onNicknameBlur" />
      </view>

      <view class="mt-4 text-xs text-gray-400"> 点击头像可修改，点击昵称可输入 </view>
    </view>
  </view>
</template>
